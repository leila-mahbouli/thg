...
{if(!empty($_POST['Nom'])AND!empty($_POST['Prenom'])AND!empty($_POST['Email'])AND!empty($_POST['Mdp'])AND!empty($_POST['Rmdp'])AND!empty($_POST['Numtel'])AND!empty($_POST['Adresse']))
    {
...
$Adresse=htmlspecialchars($_POST['Adresse']);
    ...
    { if (filter_var($Email, FILTER_VALIDATE_EMAIL)){
        $reqmail = $bdd->prepare("SELECT * FROM patient WHERE email=? ");
         ...
         if($Mdp==$Rmdp)
          {
            ...
            $insertpt =$bdd->prepare("INSERT INTO patient(tel,mdp,nom,prenom,email,adresse, confirmkey)Values(?,?,?,?,?,?,?)");
    $insertpt->execute(array($Numtel,$Mdp,$Nom,$Prenom,$Email,$Adresse,$key));
             
$header="MIME-Version: 1.0\r\n";
$header.='From:"THGAssistance.tn"<sarra.cherif.19.98@gmail.com>'."\n";
...
<html>
	<body>
		<div align="center">
 <a href="http://localhost/thg/confirmation.php?prenom='.urlencode($Prenom).'&key='.$key.'">Confirmez votre compte !</a>
		</div>
...
mail("$Email", "Confirmation de compte", $message, $header);
        ...
        else $erreur="Adresse mail déja utilisée" ;
    }
...
       <td> <input type="password" name="Rmdp"></td>
    </tr>
...
        <td ><input type="text" name="Adresse"> </td></tr>
        <br><br>
        <tr><td><center><input type="submit" name="submit" value="Je m'inscris"> </center><p class="box-register">Déjà inscrit? 
  <a href="connexion.php">Connectez-vous ici</a></p> </td></tr>
         </fieldset>
                     </form> 
                 </td></tr>
        </table>
