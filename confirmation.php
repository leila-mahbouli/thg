...
if(isset($_GET['Prenom'])AND ($_GET['key']) AND !empty($_GET['Prenom']) AND !empty($_GET['key'] )){
$Prenom= htmlspecialchars(urldecode($_GET['Prenom']));
    $key = htmlspecialchars ($_GET['key']);
$requser=$bdd->prepare("SELECT * FROM patient WHERE prenom= ? AND confirmkey = ?");    
...
if($userexist == 1){
    $user = $requser->fetch();
    if($user['confirme']== 0){
        $updateuser = $bdd->prepare("UPDATE patient SET confirme = 1 WHERE prenom= ? AND confirmkey = ?");
